...
PROMPT;
    ...
    /**
     * Refina a estratégia de um plano de estudos.
     * Retorna uma lista de disciplinas ordenadas por prioridade estratégica.
     */
    public function refinarPlano(string $concurso, string $banca, array $disciplinas): array {
        $discList = "";
        foreach($disciplinas as $d) {
            $discList .= "- " . $d['nome'] . " (Peso: " . ($d['peso_padrao'] ?? 1) . ")\n";
        }

        $prompt = <<<PROMPT
Você é um ESTRATEGISTA DE CONCURSOS PÚBLICOS.
Sua missão é reorganizar as disciplinas abaixo para o concurso {$concurso} (Banca: {$banca}).
Considere:
1. O perfil da banca {$banca} (quais matérias ela costuma pesar mais).
2. O peso oficial das disciplinas.
3. Importância estratégica para aprovação rápida.

DISCIPLINAS:
{$discList}

REQUISITO DE SAÍDA:
Retorne APENAS um JSON com o array de nomes das disciplinas na nova ordem de prioridade (da mais importante para a menos importante).
